...
# GitHub 镜像站配置（用于加速 raw.githubusercontent.com 访问）
GITHUB_MIRRORS = [
    "https://ghproxy.net/",
...
def get_github_mirror_url(original_url):
    """将 GitHub raw 地址转换为镜像站地址"""
    if "raw.githubusercontent.com" in original_url or "github.com" in original_url:
        mirror = random.choice(GITHUB_MIRRORS)
...
# IPTV-API源配置
IPTV_API_URL = get_github_mirror_url("https://raw.githubusercontent.com/gclgg/iptv-api/refs/heads/master/output/result.m3u")
IPTV_API_MAIN_GROUP = "IPTV-API源"

# 你的Logo仓库配置
LOGO_REPO_OWNER = "gclgg"
LOGO_REPO_NAME = "live"
LOGO_PATH_IN_REPO = "tv"
LOGO_BASE_URL = get_github_mirror_url(f"https://raw.githubusercontent.com/{LOGO_REPO_OWNER}/{LOGO_REPO_NAME}/main/{LOGO_PATH_IN_REPO}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url, timeout=15) as resp:
                ...
                for file_info in png_files:
                    ...
                    logo_url = get_github_mirror_url(f"{LOGO_BASE_URL}/{file_info['name']}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(HOTEL_SOURCE_URL, timeout=30) as resp:
                if resp.status != 200:
                    ...
                    return hotel_groups, hotel_group_order, hotel_update_time
...
')
        ...
        print(f"❌ 拉取失败: {e}")
        return hotel_groups, hotel_group_order, current_time
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(IPTV_API_URL, timeout=30) as resp:
                if resp.status != 200:
                    print(f"❌ 拉取失败: HTTP {resp.status}")
                    return iptv_groups, iptv_group_order, iptv_update_time
...
')
                ...
                for line in lines:
                    ...
                    if line.startswith('#EXTINF'):
                        # 解析 group-title, tvg-name, tvg-logo
                        ...
                        logo_url = logo_match.group(1).strip() if logo_match else get_logo(current_name)
                    ...
                    if line and not line.startswith('#'):
                        if current_group and current_name:
                ...
                # 尝试从M3U内容中提取更新时间戳
                iptv_update_time = current_time  # 默认使用当前时间
                for line in lines:
                    if line.startswith('# 更新时间') or line.startswith('#更新时间') or line.startswith('#Last updated'):
                        time_match = re.search(r'(\d{4}-\d{2}-\d{2}[\s:]\d{2}:\d{2}:\d{2})', line)
                        if time_match:
                            iptv_update_time = time_match.group(1)
                            break

                print(f"✅ 拉取成功，共 {len(iptv_groups)} 个分组，{total} 个频道")
                print(f"   🕐 IPTV-API源更新时间: {iptv_update_time}")

                logo_count = sum(1 for group in iptv_groups.values() for ch in group if ch['logo'])
                if logo_count > 0:
                    print(f"   🖼️ 其中 {logo_count} 个频道已有logo")

                for group in iptv_group_order:
                    if group in iptv_groups:
                        group_logo_count = sum(1 for ch in iptv_groups[group] if ch['logo'])
                        print(f"   - {group}: {len(iptv_groups[group])} 个频道 ({group_logo_count} 个有logo)")

                return iptv_groups, iptv_group_order
    except Exception as e:
        print(f"❌ 拉取失败: {e}")
        return iptv_groups, iptv_group_order, current_time
...
🕐 当前时间: {current_time}")
    ...
    # 3. 拉取酒店源
    hotel_groups, hotel_group_order, hotel_update_time = await fetch_hotel_source()

    # 4. 拉取IPTV-API源
    iptv_groups, iptv_group_order, iptv_update_time = await fetch_iptv_api_source()
...
')
        ...
        if hotel_groups and hotel_group_order:
            # 第一个分组名称加上"酒店源"前缀和时间戳
            first_group = hotel_group_order[0]
            display_first = GROUP_MAPPING.get(first_group, first_group)
            hotel_prefix = f"酒店源[{hotel_update_time}]"

            for idx, group in enumerate(hotel_group_order):
                if group in hotel_groups and hotel_groups[group]:
                    display_group = GROUP_MAPPING.get(group, group)
                    # 第一个分组加上"酒店源"前缀
                    if idx == 0:
                        display_group = f"{hotel_prefix}•{display_group}"
...
')
        ...
        if iptv_groups and iptv_group_order:
            # 第一个分组名称加上"IPTV-API源"前缀和时间戳
            first_group = iptv_group_order[0]
            display_first = GROUP_MAPPING.get(first_group, first_group)
            iptv_prefix = f"IPTV-API源[{iptv_update_time}]"

            for idx, group in enumerate(iptv_group_order):
                if group in iptv_groups and iptv_groups[group]:
                    display_group = GROUP_MAPPING.get(group, group)
                    # 第一个分组加上"IPTV-API源"前缀
                    if idx == 0:
                        display_group = f"{iptv_prefix}•{display_group}"
...
')
    ...
    iptv_logo_count = sum(1 for group in iptv_groups.values() for ch in group if ch['logo']) if iptv_groups else 0
...
📊 最终文件统计:")
    ...
    if hotel_groups:
        ...
        print(f"      源更新时间: {hotel_update_time}")
    ...
    if iptv_groups:
        ...
        print(f"      源更新时间: {iptv_update_time}")
        print(f"      其中有 {iptv_logo_count} 个频道已添加logo")
        for group in iptv_group_order:
            if group in iptv_groups:
                display_group = GROUP_MAPPING.get(group, group)
                group_logo_count = sum(1 for ch in iptv_groups[group] if ch['logo'])
                print(f"      {display_group}: {len(iptv_groups[group])} 个频道 ({group_logo_count} 个有logo)")
