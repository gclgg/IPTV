...
async def main():
    ...
    # 1. 读取本地源
    local_channels = parse_live_txt(INPUT_SOURCE)
    local_count = sum(len(local_channels[g]) for g in local_channels)
    print(f"📁 本地源: {len(local_channels)} 个分组, {local_count} 个频道")
    ...
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
        ...
        if local_channels:
            f.write(f'\n# ========== 本地源 ==========\n')
            for group, channels in local_channels.items():
                f.write(f'\n# 分组：{group}\n')
                for ch in channels:
                    logo = get_logo(ch['name'])
                    f.write(f'#EXTINF:-1 tvg-logo="{logo}" group-title="{group}",{ch["name"]}\n')
                    f.write(f'{ch["url"]}\n')
